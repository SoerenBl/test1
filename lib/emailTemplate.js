...
// fall back to the inline font stack already on the headline. Same idea for
// the recap "glass tile": box-shadow + backdrop-filter are added as inline
// styles alongside a solid background color, so a client that ignores them
// still gets a plain bordered panel, and one that understands them (Apple
// Mail again) renders the same soft-glass look the live site uses.
//
...
function headerBlock(logoUrl, heroUrl) {
  return '<tr><td>'
...
function headlineBlock(line1, line2) {
  return '<tr><td style="padding:16px 32px 6px 32px;">'
    + '<div style="font-family:Anton,Arial,\'Helvetica Neue\',sans-serif;font-weight:400;text-transform:uppercase;letter-spacing:.01em;line-height:.98;font-size:30px;color:#ffffff;">'
    + line1 + '<br>' + line2
    + '</div>'
    + '</td></tr>';
}

// The recap "glass tile": a solid panel + border for every client, with
// backdrop-filter/box-shadow layered on top as inline styles so clients
// that understand them (Apple Mail) render the same soft-glass bevel the
// live site's cards use, while everyone else just sees the flat panel.
function glassPanel(labelText, rows) {
  return '<tr><td style="padding:0 32px 26px 32px;">'
    ...
    + 'background:#38373d;background:rgba(255,255,255,0.09);'
    + 'border:1px solid rgba(255,255,255,0.22);border-radius:14px;'
    + 'backdrop-filter:blur(16px) saturate(180%);-webkit-backdrop-filter:blur(16px) saturate(180%);'
    + 'box-shadow:inset 0 1px 0 rgba(255,255,255,0.28),inset 0 -1px 0 rgba(0,0,0,0.18),inset 1px 0 0 rgba(255,255,255,0.1),inset -1px 0 0 rgba(0,0,0,0.1);">'
    + '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">'
...
function renderConfirmationEmail(opts) {
  ...
  var bodyRows = headerBlock(logoUrl, heroUrl)
    + headlineBlock(t.heroLine1, t.heroLine2)
...
function renderNotificationEmail(opts) {
  ...
  var bodyRows = headerBlock(logoUrl, heroUrl)
    + headlineBlock(NOTIFY_COPY.heroLine1, NOTIFY_COPY.heroLine2)
    + '<tr><td style="padding:18px 32px 0 32px;font-family:Helvetica,Arial,sans-serif;">'
